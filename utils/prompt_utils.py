...
def iterative_enricher_few_shot_prep(few_shot_data_path: str, q_id: int, q_db_id: str, level_shot_number: str, schema_existance: bool, enrichment_level: str, mode: str) -> str:
    """
    The function selects the given number of exemple from the question enrichment few-shot data, and then
    concatenate the selected exemples in string format. 
    - The few-shot examples will be selected from the set of data contains databases different than the considered question.
...
def extract_iterative_enricher_prompt_template(template_path: str) -> str:
    with open(template_path, "r", encoding="utf-8") as f:
        return f.read()


def fill_iterative_enricher_prompt_template(
    template: str,
    schema: str,
    db_samples: str,
    question: str,
    ...
    """
    Same as fill_question_enrichment_prompt_template, plus the enriched question (v1)
    and its enrichment reasoning (v1) are appended into the prompt.
    """
    ...
    # extra parts for iterative enrichment
    enriched_question_v1 = "\n### Enriched Question (v1): \n" + enriched_question_v1
    enrichment_reasoning_v1 = "\n### Enrichment Reasoning (v1): \n" + enrichment_reasoning_v1
